...
proc ::crelay::init {args} {
    ...
    putlog "CHANRELAY $::crelay::version launched"
}
...
namespace eval crelay {
    ...
    proc trans:who {nick uhost handle chan args} {
        if { [lindex [split $args] 1] != "" } {
            ...
            if { $him == -1 } {
                putserv "PRIVMSG $nick :$args est un r seau inconnu";
                return 0
            } else {
    ...
    proc recv:act {frm_bot command arg} {
        if {[set him [lsearch $::crelay::eggdrops $frm_bot]] >= 0} {
            ...
            [namespace current]::cr:log p "$::crelay::me(chan)" "Action: [join [lrange $argl 1 end]]"
        }
        return 0
    }
